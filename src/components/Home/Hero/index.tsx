...
const Hero = () => {
    ...
    const handleClickOutside = useCallback(
        (event: MouseEvent) => {
            if (
                BuyRef.current &&
                !BuyRef.current.contains(event.target as Node)
            ) {
                setIsBuyingOpen(false);
            }
            if (
                SellRef.current &&
                !SellRef.current.contains(event.target as Node)
            ) {
    ...
    return (
        <section
            className="relative md:pt-40 md:pb-28 py-20 overflow-hidden z-1 height-full"
            id="main-banner"
        >
                ...
                <div className="grid grid-cols-12">
                    ...
                    >
                        <h1 className="font-medium lg:text-40 md:text-25 text-20 lg:text-start text-center text-white mb-8 mt-14">
                            HAZ QUE TU{' '}
                            <span className="text-primary">DINERO</span> TRABAJE
                            POR TI
                        </h1>
